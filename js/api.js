...
export const api = {
  ...
  clientCodeOrganizations: (body) => request("/auth/client-code/organizations", { method: "POST", body: JSON.stringify(body) }),
  startClientCodeAuth: (body) => request("/auth/client-code/start", { method: "POST", body: JSON.stringify(body) }),
  ...
  deleteMasterService: (id) => request(`/master-services/pairs/${id}`, { method: "DELETE" }),
  ...
  inventories: (orgId) => request(`/organizations/${orgId}/inventories`, { cache: "no-store" }),
  createInventory: (orgId, body) => request(`/organizations/${orgId}/inventories`, { method: "POST", body: JSON.stringify(body) }),
  inventory: (orgId, inventoryId) => request(`/organizations/${orgId}/inventories/${inventoryId}`, { cache: "no-store" }),
  updateInventoryItems: (orgId, inventoryId, body) => request(`/organizations/${orgId}/inventories/${inventoryId}/items`, { method: "PATCH", body: JSON.stringify(body) }),
  ...
  bookingCompatibility: (orgId, branchId, masterId, serviceId, bookingDate) => request(`/booking-api/public-booking/compatibility?organization_id=${orgId}&branch_id=${branchId}&master_id=${masterId}&service_ids=${serviceId}&booking_date=${encodeURIComponent(bookingDate)}`),
  ...
  uploadPushNotificationImages: (files) => {
    const data = new FormData();
    [...files].forEach((file) => data.append("files", file));
