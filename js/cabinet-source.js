/* Source for the legacy-compatible cabinet bundle. Rebuild with scripts/build-cabinet-legacy.ps1. */
if (!String.prototype.padStart) {
  String.prototype.padStart = function padStart(targetLength, fillString) {
    ...
    const fill = String(fillString === undefined ? " " : fillString);
...
const form = document.querySelector("[data-cabinet-form]");
      ...
      const pushToggle = document.querySelector("[data-cabinet-push-toggle]");
      const pushStatus = document.querySelector("[data-cabinet-push-status]");
      const contactPreference = document.querySelector("[data-cabinet-contact-preference]");
      const notificationsList = document.querySelector("[data-cabinet-notifications-list]");
      const achievementsList = document.querySelector("[data-cabinet-achievements]");
      const achievementModal = document.querySelector("[data-cabinet-achievement-modal]");
      const achievementContent = document.querySelector("[data-cabinet-achievement-content]");
      const clientBookingForm = document.querySelector("[data-client-booking-form]");
      const clientBookingServices = document.querySelector("[data-client-booking-services]");
      const clientBookingSlots = document.querySelector("[data-client-booking-slots]");
      const clientBookingMessage = document.querySelector("[data-client-booking-message]");
      const clientBookingList = document.querySelector("[data-client-booking-list]");
      const publicBookingMatch = window.location.pathname.match(/^\/online-booking\/(\d+)\/?$/);
      const publicBookingOrganizationId = publicBookingMatch ? Number(publicBookingMatch[1]) : 0;
      const publicBookingRegistration = document.querySelector("[data-public-booking-registration]");
      const publicBookingRegistrationForm = document.querySelector("[data-public-booking-registration-form]");
      ...
      let preferredContactChannel = "";
      const allCabinetSections = ["profile", "visits", "loyalty", "achievements", "referrals"];
      const loadedCabinetSections = new Set();
      let cabinetSectionsPromise = null;
      let selectedVisit = null;
      let visitsPage = 1;
      let visitsExpanded = false;
      let bookingVisitsExpanded = false;
      let branchReviewsRequestId = 0;
      let openServiceCategoryId = null;
      let visitReviewRating = 0;
      const reviewedVisitIds = new Set(JSON.parse(sessionStorage.getItem("cabinet.reviewedVisitIds") || "[]"));
      ...
      let clientBookingOptions = null;
      let publicBookingAnonymous = false;
      let publicBookingClientId = null;
      let publicBookingVisitClientName = "";
      let clientBookingMasterPreviews = null;
      let clientBookingMastersRequestId = 0;
      let clientBookingMastersLoadingKey = "";
      let clientBookingCompatibility = null;
      let clientBookingCompatibilityRequestId = 0;
      let clientBookingSelectedStart = "";
      let clientBookingCalendarMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
      const SESSION_TOKEN_KEY = "loyalty.sessionToken";
      const CABINET_NOTIFICATIONS_SEEN_KEY = "cabinetNotificationsSeenAt";
      const PWA_PUSH_PERMISSION_REQUESTED_KEY = "loyalty.pwaPushPermissionRequested.v2";
      const PWA_SESSION_HASH_PREFIX = "#pwa_session=";
      ...
      function showBookingSuccessToast(message = "\u0412\u044b \u0443\u0441\u043f\u0435\u0448\u043d\u043e \u0437\u0430\u043f\u0438\u0441\u0430\u043d\u044b") {
      ...
      let serviceGalleryUrls = [];
      let serviceGalleryIndex = 0;
      let serviceGalleryTitle = "";
      ...
      function openServiceGallery(imageUrls, title, startIndex = 0) {
        ...
        document.querySelector("[data-service-gallery-close]")?.focus();
      }

      function closeServiceGallery() {
        document.querySelector("[data-service-gallery-lightbox]")?.remove();
      ...
      const CABINET_ORGANIZATION_BRANDING_CACHE_PREFIX = "cabinet.organizationBranding:";
      let cabinetOrganizationBranding = null;
      ...
      async function refreshPushState() {
        ...
        contactPreference.value = preferredOption?.value || "";
      ...
      function notificationImageGroup(item) {
        const images = [...new Set((Array.isArray(item?.image_urls) ? item.image_urls : []).filter(Boolean))].slice(0, 10);
      ...
      function setNotificationBadge(hasUnread) {
        document.querySelector("[data-cabinet-notifications-open]")?.classList.toggle("has-unread", !!hasUnread);
      }
      ...
      async function requestPushOnFirstPwaLaunch(state) {
        ...
        if (Notification.permission !== "granted") {
          ...
          pushStatus.textContent = Notification.permission === "denied"
            ? "Уведомления запрещены в настройках приложения."
            : "Включите уведомления тумблером, чтобы iPhone показал системный запрос.";
        }
      }
      ...
      function cabinetBranchTimezone(branchId) {
        return clientBookingOptions?.branches?.find((branch) => String(branch.id) === String(branchId))?.timezone || "Europe/Moscow";
      }

      function dateTime(value, branchId = null) {
        if (!value) return "";
        const date = new Date(value);
        return Number.isNaN(date.getTime()) ? String(value) : date.toLocaleString("ru-RU", branchId ? { timeZone: cabinetBranchTimezone(branchId) } : undefined);
      }
      ...
      function visitDateTime(value, branchId) {
        ...
        return Number.isNaN(date.getTime()) ? String(value) : date.toLocaleString("ru-RU", {
          timeZone: cabinetBranchTimezone(branchId), year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit",
        });
      }
      ...
      function bonusName(section) {
        const code = bonusBalanceForSection(section)?.bonus_type || section.replace(/^bonus_/, "") || "cashback";
      ...
      function visitEmployeeId(visit) {
        return Number(visit.employee_id || visit.master_id || visit.yclients_staff?.id || 0);
      }

      function visitCommentPhotos(visit) {
        const photos = Array.isArray(visit?.photos_comment) ? visit.photos_comment : [];
        if (!photos.length || !visit?.id) return "";
      ...
      function visitHistoryItems(item) {
        ...
        const services = (item.services || visit.yclients_services || []).length
          ? (item.services || visit.yclients_services || [])
          : parsed.serviceNames.split(",").map((name) => ({ name: name.trim() })).filter((service) => service.name);
        const products = (item.products || visit.yclients_goods_transactions || []).length
          ? (item.products || visit.yclients_goods_transactions || [])
          : parsed.productNames.split(",").map((name) => ({ name: name.trim() })).filter((product) => product.name);
      ...
      function renderVisitsPage() {
        ...
        wrapper.innerHTML = visitsPageHtml(cabinetData?.visits || []).trim();
      ...
      function sectionBody(section) {
        ...
        if (section === "client_visits") {
          return visitsPageHtml(cabinetData?.visits || []);
        }
        if (section === "client_personal_link") {
          ...
          return `
            ${referralLink
              ? `<div class="inline-form compact"><label><span>\u0420\u0435\u0444\u0435\u0440\u0430\u043b\u044c\u043d\u0430\u044f \u0441\u0441\u044b\u043b\u043a\u0430</span><input value="${escapeHtml(referralLink)}" readonly></label><button type="button" class="ghost" data-cabinet-copy-referral aria-label="\u041a\u043e\u043f\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u0440\u0435\u0444\u0435\u0440\u0430\u043b\u044c\u043d\u0443\u044e \u0441\u0441\u044b\u043b\u043a\u0443" title="\u041a\u043e\u043f\u0438\u0440\u043e\u0432\u0430\u0442\u044c"><img src="/fronted/icons/copy.svg" alt="\u041a\u043e\u043f\u0438\u0440\u043e\u0432\u0430\u0442\u044c"></button></div>`
      ...
      function applyCabinetBottomNavOrder(sections) {
        ...
        const configuredSections =
          cabinetAccessSections(sections);
        ...
        items
          .sort((a, b) => {
            const aSection =
              a.dataset.cabinetBottomSection;

            const bSection =
              b.dataset.cabinetBottomSection;

            const aDefault =
              clientBottomMenuSections.indexOf(
                aSection
              );
      ...
      function bookingScheduleLabel(schedule) {
        ...
        const entries = Object.entries(schedule)
          .filter(([day, value]) => days[day] && value && typeof value === "object")
          .map(([day, value]) => `${days[day]} ${value.closed ? "\u0432\u044b\u0445\u043e\u0434\u043d\u043e\u0439" : `${value.from || "09:00"}\u2013${value.to || "18:00"}`}`);
        return entries.length ? entries.join(" \u00b7 ") : "\u0413\u0440\u0430\u0444\u0438\u043a \u043d\u0435 \u0443\u043a\u0430\u0437\u0430\u043d";
      }
      ...
      function renderClientBookingLogo(branch = null) {
        ...
        const organizationId = Number(
          clientBookingOptions?.organization_id
          || publicBookingOrganizationId
          || cabinetData?.organization_id
          || cabinetData?.client?.organization_id
          || currentClient?.organization_id
          || 0
        );
        const photoUrl = branch?.id
          ? `/organizations/branches/${encodeURIComponent(branch.id)}/photo-file?v=${encodeURIComponent(branch.photo_file_id || "7-photo")}`
          : organizationId
            ? `/organizations/${encodeURIComponent(organizationId)}/photo-file?v=${encodeURIComponent(cabinetOrganizationBranding?.updated_at || cabinetOrganizationBranding?.photo_file_id || "7-photo")}`
            : "";
      ...
      async function renderClientBookingBranchMeta() {
        ...
        const branch = clientBookingOptions?.branches?.find((item) => Number(item.id) === branchId);
        ...
        const organizationId = clientBookingOptions?.organization_id || publicBookingOrganizationId || cabinetData?.organization_id || cabinetData?.client?.organization_id || currentClient?.organization_id;
        ...
        try {
          ...
          meta.querySelector("[data-cabinet-branch-reviews-count]").textContent = reviewsCountLabel(rating?.reviews_count || 0);
        } catch {
          if (requestId !== branchReviewsRequestId) return;
      ...
      function bookingServicePriceLabel(item) {
        const minimum = item.price_min ?? item.price;
        const maximum = item.price_max ?? item.price;
        if (minimum === null || minimum === undefined) return "";
        if (maximum !== null && maximum !== undefined && Number(maximum) !== Number(minimum)) {
      ...
      function isSelectedMasterBookingDateBlocked(iso) {
        ...
        const master = (clientBookingOptions?.masters || []).find((item) => String(item.id) === String(masterId));
        if (!master) return false;
        const unavailableByShift = Array.isArray(master.available_dates)
          && !master.available_dates.includes(iso);
        return unavailableByShift || (master.booking_blocks || []).some((block) =>
          !block.time_from && !block.time_to
          && String(block.date_from || "") <= iso
          && iso <= String(block.date_to || block.date_from || "")
        );
      }
      ...
      function renderClientBookingCalendar() {
        ...
        const minDate = input.min ? new Date(`${input.min}T00:00:00`) : new Date();
        const maxDate = input.max ? new Date(`${input.max}T00:00:00`) : new Date(Date.now() + 30 * 86400000);
        ...
        clientBookingForm.querySelector("[data-booking-calendar-title]").textContent =
        ...
        clientBookingForm.querySelector("[data-booking-calendar-days]").innerHTML = Array.from({ length: 42 }, (_, index) => {
          ...
          const today = iso === bookingDateIso(new Date());
        ...
        clientBookingForm.querySelector("[data-booking-calendar-prev]").disabled =
          previousMonth < new Date(minDate.getFullYear(), minDate.getMonth(), 1);
        clientBookingForm.querySelector("[data-booking-calendar-next]").disabled =
      ...
      function setClientBookingStep(step, forceOpen) {
        ...
        if (!selectedPanel || selectedTrigger?.disabled) return;
        const shouldOpen = forceOpen ?? selectedPanel.hidden;
        ...
        if (shouldOpen && step === "services") {
          clientBookingForm.querySelector("[data-client-booking-service-search]")?.focus();
        }
      }

      function updateClientBookingSummaries() {
        const serviceIds = new Set(selectedBookingServiceIds());
        const selectedServices = (clientBookingOptions?.services || []).filter((item) => serviceIds.has(Number(item.id)));
        const serviceSummary = clientBookingForm.querySelector("[data-booking-service-summary]");
        serviceSummary.textContent = selectedServices.length
          ? (selectedServices.length === 1 ? selectedServices[0].title : `Выбрано услуг: ${selectedServices.length}`)
          : "Выбрать услуги";
        const masterInput = clientBookingForm.elements.master_id;
        const selectedMaster = (clientBookingOptions?.masters || []).find((item) => String(item.id) === masterInput.value);
        clientBookingForm.querySelector("[data-booking-master-summary]").textContent =
          selectedMaster?.name || "Выбрать специалиста";
        ...
        const selectedTime = selectedSlot?.nextElementSibling?.textContent?.trim() || "";
        const dateLabel = dateValue
          ? new Date(`${dateValue}T00:00:00`).toLocaleDateString("ru-RU", { day: "numeric", month: "long" })
          : "";
        clientBookingForm.querySelector("[data-booking-date-summary]").textContent =
          dateLabel ? `${dateLabel}${selectedTime ? `, ${selectedTime}` : ""}` : "Выбрать дату и время";
      }

      function visitServiceTitle(item) {
        const visit = item?.visit || item || {};
        const line = String(visit.comment || "").split("\n").find((value) => value.startsWith("__services:"));
        return line?.slice("__services:".length).trim() || "Визит";
      }

      function renderClientBookingList() {
        const now = Date.now();
        const visits = [...(cabinetData?.visits || [])].sort((left, right) => {
        ...
        clientBookingList.innerHTML = visits.length
          ? `<div class="cabinet-visit-list">${visits.map(visitHistoryItems).join("")}</div>`
          : '<p class="cabinet-history-empty">Записей пока нет.</p>';
      }

      function renderClientBookingServices() {
        const branchId = Number(clientBookingForm.elements.branch_id.value || 0);
        const selectedIds = new Set(selectedBookingServiceIds());
        const compatibleServiceIds = clientBookingSelectedStart && clientBookingCompatibility
          ? new Set(clientBookingCompatibility.service_ids.map(Number))
          : null;
        const query = String(clientBookingForm.querySelector("[data-client-booking-service-search]")?.value || "").trim().toLocaleLowerCase("ru");
        const services = (clientBookingOptions?.services || []).filter((item) =>
          (!branchId || item.branch_ids.includes(branchId))
          && (!compatibleServiceIds || compatibleServiceIds.has(Number(item.id)))
          && (!query || String(item.title || "").toLocaleLowerCase("ru").includes(query))
        );
      ...
      function renderClientBookingMasters() {
        ...
        const compatibleMasterIds = clientBookingSelectedStart && clientBookingCompatibility
          ? new Set(clientBookingCompatibility.master_ids.map(Number))
          : null;
        const availableMasters = (clientBookingOptions?.masters || []).filter((item) =>
          (!branchId || item.branch_ids.includes(branchId))
          && (!serviceIds.length || serviceIds.every((id) => item.service_ids.includes(id)))
          && (!compatibleMasterIds || compatibleMasterIds.has(Number(item.id)))
        );
        const masterInput = clientBookingForm.elements.master_id;
        ...
        const masters = mastersLoading
          ? []
          : clientBookingMasterPreviews.filter((item) => availableIds.has(Number(item.id)));
        const container = clientBookingForm.querySelector("[data-client-booking-masters]");
        container.innerHTML = mastersLoading
          ? '<p class="cabinet-history-empty">\u0417\u0430\u0433\u0440\u0443\u0436\u0430\u0435\u043c \u0441\u043f\u0435\u0446\u0438\u0430\u043b\u0438\u0441\u0442\u043e\u0432\u2026</p>'
          : masters.length ? masters.map((item) => {
            ...
            const nearestDate = slots?.[0]?.date
              ? new Date(`${slots[0].date}T00:00:00`).toLocaleDateString("ru-RU", { weekday: "short", day: "numeric", month: "long" })
              : "";
            return `<article class="cabinet-booking-master${String(item.id) === masterInput.value ? " selected" : ""}" data-booking-master-card="${escapeHtml(item.id)}">
              ...
              <div class="cabinet-booking-master-nearest">
                ${slots === null ? '<small>Ищем ближайшее время…</small>' : slots.length
                  ? `<small>Ближайшее время для записи, ${escapeHtml(nearestDate)}${slots[0].branch_name ? ` · ${escapeHtml(slots[0].branch_name)}` : ""}:</small><span><button type="button" data-booking-nearest-slot data-master-id="${escapeHtml(item.id)}" data-branch-id="${escapeHtml(slots[0].branch_id || "")}" data-start="${escapeHtml(slots[0].start)}">${escapeHtml(slots[0].time)}</button></span>`
                  : '<small>На ближайшие дни свободного времени нет.</small>'}
              </div>
            </article>`;
      ...
      async function loadClientBookingMasters() {
        ...
        try {
          ...
          clientBookingForm.querySelector("[data-client-booking-masters]").innerHTML =
            `<p class="cabinet-message error">${escapeHtml(error.message || "?? ??????? ????????? ????????????.")}</p>`;
        }
      }
      ...
      async function loadClientBookingOptions() {
        ...
        try {
          const optionsPath = publicBookingOrganizationId
            ? `/booking-api/public-booking/options?organization_id=${encodeURIComponent(publicBookingOrganizationId)}`
            : "/booking-api/client-booking/options";
          ...
          clientBookingForm.querySelectorAll("[data-booking-step-toggle]").forEach((trigger) => { trigger.disabled = false; });
      ...
      function renderAchievements() {
        const all = cabinetData?.organization_achievements || [];
        const received = cabinetData?.achievements || [];
      ...
      async function openEmployeeReviews(employeeId, employeeName, details = {}) {
        ...
        try {
          const reviews = await requestJson(`/crm-api/client-reviews?organization_id=${encodeURIComponent(clientBookingOptions?.organization_id)}&employee_id=${encodeURIComponent(employeeId)}&limit=200`, { cache: "no-store" });
          ...
          (clientBookingOptions?.masters || []).forEach(updateMaster);
          renderClientBookingMasters();
          list.innerHTML = reviewsCount
            ? reviews.map((review) => { const name = review.client_name || "\u041a\u043b\u0438\u0435\u043d\u0442"; return `<article class="cabinet-employee-review"><header><div><b>${escapeHtml(name)}</b><span><span class="cabinet-review-stars">${reviewStars(review.rating)}</span><small>${escapeHtml(reviewDateLabel(review.created_at || review.date))}</small></span></div></header>${review.text ? `<p>${escapeHtml(review.text)}</p>` : ""}</article>`; }).join("")
            : '<p class="cabinet-history-empty">\u041e\u0442\u0437\u044b\u0432\u043e\u0432 \u043f\u043e\u043a\u0430 \u043d\u0435\u0442.</p>';
        } catch {
      ...
      async function openBranchReviews(branch) {
        ...
        const detailsMarkup = Array.from(sourceMeta?.children || []).slice(0, 2).map((item) => item.outerHTML).join("");
        profile.innerHTML = `<span class="cabinet-booking-logo cabinet-branch-reviews-logo">${sourceLogo?.innerHTML || "<span>L</span>"}</span><div class="cabinet-branch-reviews-info"><small>Онлайн-запись</small><h1>${escapeHtml(branch.name || "Филиал")}</h1><div class="cabinet-branch-reviews-details">${detailsMarkup}</div></div>`;
        list.innerHTML = '<p class="cabinet-history-empty">Загружаем отзывы…</p>';
        ...
        try {
          ...
          list.innerHTML = reviews.length
            ? reviews.map((review) => `<article class="cabinet-employee-review"><header><div><b>${escapeHtml(review.client_name || "Клиент")}</b><span><span class="cabinet-review-stars">${reviewStars(review.rating)}</span><small>${escapeHtml(reviewDateLabel(review.created_at || review.date))}</small></span></div></header>${review.text ? `<p>${escapeHtml(review.text)}</p>` : ""}</article>`).join("")
            : '<p class="cabinet-history-empty">Отзывов пока нет.</p>';
        } catch {
          list.innerHTML = '<p class="cabinet-history-empty">Не удалось загрузить отзывы.</p>';
        }
      ...
      function syncPublicBookingGuestUi() {
        ...
        if (bottomNav) {
          bottomNav.hidden = hideBottomNav
            || document.body.classList.contains("cabinet-registration-mode");
        }
      }
      ...
      function renderCabinetProfilePhoto(user) {
        ...
        initials.textContent = [user?.first_name, user?.last_name]
          .filter(Boolean)
          .map((part) => String(part).trim().charAt(0))
          .join("")
          .slice(0, 2)
          .toUpperCase() || "К";
        if (user?.id && user?.photo_file_id) {
      ...
      document.querySelector("[data-cabinet-profile-photo-input]")?.addEventListener("change", async (event) => {
      ...
      async function consumeLink() {
        ...
        if (publicBookingOrganizationId) {
          ...
          const sessionOrganizationId = Number(cabinetData?.organization_id || currentClient?.organization_id || 0);
        ...
        try {
          ...
          document.querySelector(".cabinet-shell")?.setAttribute("hidden", "");
      ...
      document.addEventListener("click", async (event) => {
        ...
        if (visitsToggle) {
          ...
          visitsToggle.closest(".cabinet-history-card--visits")?.classList.toggle("is-expanded", visitsExpanded);
          return;
        }
        ...
        if (bookingVisitsToggle) {
          ...
          bookingVisitsToggle.closest(".cabinet-my-bookings")?.classList.toggle("is-expanded", bookingVisitsExpanded);
          return;
        }
        ...
        if (achievementButton) {
          const achievement = (cabinetData?.organization_achievements || []).find((item) => String(item.id) === achievementButton.dataset.cabinetAchievement);
        ...
        if (visitButton) {
          selectedVisit = (cabinetData?.visits || []).find((item) => String((item.visit || item).id) === String(visitButton.dataset.cabinetVisit)) || null;
      ...
      contactPreference?.addEventListener("change", async () => {
        const selectedOption = contactPreference.selectedOptions[0];
        const previousChannel = preferredContactChannel;
        preferredContactChannel = selectedOption?.dataset.cabinetContactChannel || "";
        const { organizationId, clientId } = pushContext();
        ...
        try {
          ...
          contactPreference.value = previousOption?.value || "";
      ...
      document.querySelector("[data-visit-review-form]").addEventListener("submit", async (event) => {
        ...
        const message = reviewForm.querySelector("[data-visit-review-message]");
        const submit = reviewForm.querySelector("[data-visit-review-submit]");
        const organizationId = clientBookingOptions?.organization_id || cabinetData?.organization_id || currentClient?.organization_id;
        const clientId = clientBookingOptions?.client_id || cabinetData?.client_id || currentClient?.id;
        ...
        message.textContent = "\u041f\u0443\u0431\u043b\u0438\u043a\u0443\u0435\u043c \u043e\u0442\u0437\u044b\u0432\u2026";
        try {
          ...
          renderClientBookingMasters();
          renderHistorySections(currentCardSections);
          message.textContent = "\u041e\u0442\u0437\u044b\u0432 \u043e\u043f\u0443\u0431\u043b\u0438\u043a\u043e\u0432\u0430\u043d.";
          setBookingLoader(false);
          await showBookingSuccessToast("\u041e\u0442\u0437\u044b\u0432 \u0443\u0441\u043f\u0435\u0448\u043d\u043e \u043e\u043f\u0443\u0431\u043b\u0438\u043a\u043e\u0432\u0430\u043d");
          setCabinetTab("profile");
        } catch {
          message.textContent = "\u041d\u0435 \u0443\u0434\u0430\u043b\u043e\u0441\u044c \u043e\u043f\u0443\u0431\u043b\u0438\u043a\u043e\u0432\u0430\u0442\u044c \u043e\u0442\u0437\u044b\u0432. \u041f\u043e\u043f\u0440\u043e\u0431\u0443\u0439\u0442\u0435 \u0435\u0449\u0451 \u0440\u0430\u0437.";
      ...
      clientBookingForm.addEventListener("change", async (event) => {
        ...
        if (event.target.name === "branch_id") {
          ...
          renderClientBookingBranchMeta();
          clientBookingForm.querySelectorAll('[name="service_ids"]').forEach((input) => { input.checked = false; });
        ...
        if (event.target.name === "starts_at") {
          ...
          await loadClientBookingSlots();
          clientBookingForm.querySelector("[data-client-booking-submit]").disabled =
            !selectedBookingServiceIds().length || !clientBookingForm.elements.master_id.value;
          updateClientBookingSummaries();
        }
      });
      ...
      clientBookingForm.addEventListener("click", async (event) => {
        ...
        if (thumbnail) {
          ...
          const mainImage = gallery?.querySelector(".cabinet-booking-service-gallery-main img");
          const source = thumbnail.querySelector("img")?.src;
          if (mainImage && source) mainImage.src = source;
          gallery?.querySelectorAll("[data-booking-service-thumbnail]").forEach((item) => item.classList.toggle("active", item === thumbnail));
          return;
        }
        const galleryButton = event.target.closest("[data-service-gallery-open]");
        if (galleryButton) {
        ...
        if (categoryTrigger) {
          ...
          clientBookingForm.querySelectorAll("[data-booking-service-category]").forEach((category) => {
            ...
            category.querySelector("[data-booking-service-category-toggle]")?.setAttribute("aria-expanded", String(isCurrent));
          });
          return;
        }
        ...
        if (nearestSlot) {
          ...
          clientBookingCalendarMonth = new Date(`${nearestSlot.dataset.start.slice(0, 7)}-01T00:00:00`);
          renderClientBookingMasters();
          renderClientBookingCalendar();
          updateClientBookingSummaries();
          setClientBookingStep("datetime", true);
          await loadClientBookingSlots();
          const matchingSlot = [...clientBookingForm.querySelectorAll('[name="starts_at"]')]
            .find((input) => input.value === nearestSlot.dataset.start);
          if (matchingSlot) {
        ...
        if (dateButton?.dataset.bookingDateBlocked === "true") {
          showSelectedMasterDateUnavailable();
          return;
        }
      ...
      clientBookingForm.addEventListener("submit", async (event) => {
        event.preventDefault();
        const startsAt = clientBookingForm.querySelector('[name="starts_at"]:checked')?.value;
        const branchId = Number(clientBookingForm.elements.branch_id.value || 0);
      ...
      publicBookingRegistrationForm.addEventListener("submit", async (event) => {
        event.preventDefault();
        const message = publicBookingRegistrationForm.querySelector("[data-public-booking-registration-message]");
      ...
      form.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          const endpoint = token
            ...
            : currentClient?.id
              ? `/crm-api/clients-core/clients/${encodeURIComponent(currentClient.id)}`
              : "";
