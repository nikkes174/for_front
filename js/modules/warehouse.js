...
export function bindWarehouse(root, ctx) {
  ...
  root.addEventListener("input", (e) => {
    ...
    if (productSearch) { const { content, state } = storageProductPickerStateFrom(productSearch); if (!content || !state) return; state.productQuery = productSearch.value || ""; state.page = 1; const products = content.querySelector("[data-storage-picker-products]"); if (products) products.innerHTML = renderStoragePickerProducts(state); return; }
  });
  root.addEventListener("change", (e) => {
    ...
    const counter = content.querySelector("[data-storage-picker-selected-count]"); if (counter) counter.textContent = `\u0412\u044b\u0431\u0440\u0430\u043d\u043e: ${state.selectedProducts.size}`;
    const submit = content.querySelector("[data-storage-picker-submit]"); if (submit) submit.disabled = state.selectedProducts.size === 0;
  });
