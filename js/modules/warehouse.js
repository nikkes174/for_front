...
function createStorageProductPickerState() { return { categories: [], products: [], existingProductIds: new Set(), selectedProducts: new Map(), categoryQuery: "", productQuery: "", selectedCategoryId: null, page: 1 }; }
...
function storageProductPickerAmount(state, productId) { return state.selectedProducts.get(String(productId)) ?? 1; }
function productCountLabel(count) { const mod100 = count % 100; const mod10 = count % 10; if (mod100 >= 11 && mod100 <= 14) return "товаров"; if (mod10 === 1) return "товар"; if (mod10 >= 2 && mod10 <= 4) return "товара"; return "товаров"; }
function storageProductPickerSelectionState(state) { const products = storageProductPickerProducts(state); if (!products.length) return { products, selectedCount: 0, allSelected: false, someSelected: false }; const selectedCount = products.filter((product) => state.selectedProducts.has(String(product.id))).length; return { products, selectedCount, allSelected: selectedCount === products.length, someSelected: selectedCount > 0 && selectedCount < products.length }; }
function renderStoragePickerSelectAll(state) { const result = storageProductPickerSelectionState(state); if (!result.products.length) return ""; return '<label class="warehouse-product-picker-select-all"><input type="checkbox" data-storage-picker-select-all' + (result.allSelected ? " checked" : "") + '><span>Выбрать все</span><small>' + result.products.length + " " + productCountLabel(result.products.length) + "</small></label>"; }
function syncStoragePickerSelectAll(content, state) { const checkbox = content.querySelector("[data-storage-picker-select-all]"); if (!checkbox) return; const result = storageProductPickerSelectionState(state); checkbox.checked = result.allSelected; checkbox.indeterminate = result.someSelected; }
function syncStoragePickerFooter(content, state) { const counter = content.querySelector("[data-storage-picker-selected-count]"); const submit = content.querySelector("[data-storage-picker-submit]"); if (counter) counter.textContent = "Выбрано: " + state.selectedProducts.size; if (submit) submit.disabled = state.selectedProducts.size === 0; syncStoragePickerSelectAll(content, state); }
function renderStoragePickerProductList(content, state) { const products = content.querySelector("[data-storage-picker-products]"); if (!products) return; const selectAll = content.querySelector(".warehouse-product-picker-select-all"); const markup = renderStoragePickerSelectAll(state); if (selectAll) selectAll.outerHTML = markup; else if (markup) products.insertAdjacentHTML("beforebegin", markup); products.innerHTML = renderStoragePickerProducts(state); syncStoragePickerFooter(content, state); }
function renderStoragePickerProducts(state) { const { products, totalPages, pageItems } = storageProductPickerPageData(state); if (!products.length) return `<p class="empty">Товары не найдены.</p>`; return `<div class="warehouse-product-picker-list">${pageItems.map((product) => { const productId = String(product.id); const selected = state.selectedProducts.has(productId); return `<div class="warehouse-product-picker-item"><input type="checkbox" class="warehouse-product-picker-checkbox" value="${Number(product.id)}" data-storage-picker-product${selected ? " checked" : ""} aria-label="Выбрать товар"><span class="warehouse-product-picker-item-title" title="${esc(product.title)}">${esc(product.title)}</span><input type="number" class="warehouse-product-picker-amount" min="0.01" step="0.01" value="${esc(storageProductPickerAmount(state, product.id))}" data-storage-picker-product-amount="${Number(product.id)}" aria-label="Количество"${selected ? "" : " disabled"}></div>`; }).join("")}</div>${totalPages > 1 ? `<div class="warehouse-product-picker-pagination"><button type="button" class="btn btn-outline-secondary" data-storage-picker-page="prev"${state.page <= 1 ? " disabled" : ""}>Назад</button><span>${state.page} / ${totalPages}</span><button type="button" class="btn btn-outline-secondary" data-storage-picker-page="next"${state.page >= totalPages ? " disabled" : ""}>Вперёд</button></div>` : ""}`; }
function renderStorageProductPicker(target, state) { if (!target) return; target.innerHTML = `<div class="warehouse-product-picker-fields"><div class="warehouse-product-picker-category-section"><label class="warehouse-product-picker-search"><span>Категория</span><input type="search" placeholder="Найти категорию" autocomplete="off" value="${esc(state.categoryQuery)}" data-storage-picker-category-search></label><div class="warehouse-product-picker-categories" data-storage-picker-categories>${renderStoragePickerCategories(state)}</div></div>${state.selectedCategoryId ? `<div class="warehouse-product-picker-product-section"><label class="warehouse-product-picker-search"><span>Товар</span><input type="search" placeholder="Найти товар" autocomplete="off" value="${esc(state.productQuery)}" data-storage-picker-product-search></label>${renderStoragePickerSelectAll(state)}<div data-storage-picker-products>${renderStoragePickerProducts(state)}</div></div>` : `<p class="empty warehouse-product-picker-hint">Выберите категорию товаров.</p>`}</div><div class="warehouse-product-picker-footer"><span data-storage-picker-selected-count>Выбрано: ${state.selectedProducts.size}</span><button type="button" class="primary standard-save-button" data-storage-picker-submit${state.selectedProducts.size ? "" : " disabled"}>Добавить</button></div><p data-storage-picker-message></p>`; syncStoragePickerSelectAll(target, state); target._storageProductPickerState = state; }
...
export function bindWarehouse(root, ctx) {
  ...
  root.addEventListener("click", async (e) => {
    ...
    if (pageButton) { const { content, state } = storageProductPickerStateFrom(pageButton); if (!content || !state) return; const { totalPages } = storageProductPickerPageData(state); state.page += pageButton.dataset.storagePickerPage === "next" ? 1 : -1; state.page = Math.max(1, Math.min(state.page, totalPages)); const products = content.querySelector("[data-storage-picker-products]"); if (products) products.innerHTML = renderStoragePickerProducts(state); syncStoragePickerFooter(content, state); return; }
    const submit = e.target.closest("[data-storage-picker-submit]");
    if (submit) { const picker = submit.closest("[data-storage-product-picker]"); const content = picker?.querySelector("[data-storage-product-picker-content]"); const state = content?._storageProductPickerState; if (!picker || !content || !state || !state.selectedProducts.size) return; const items = [...state.selectedProducts.entries()].map(([productItemId, amount]) => ({ product_item_id: Number(productItemId), amount: Number(amount) })); const message = content.querySelector("[data-storage-picker-message]"); if (!items.length || items.some((item) => !item.product_item_id || !Number.isFinite(item.amount) || item.amount <= 0)) { if (message) message.textContent = "Укажите количество больше нуля для всех выбранных товаров."; return; } submit.disabled = true; try { await api.addStorageProducts(Number(picker.dataset.storageId), { items }); root.querySelector("[data-warehouse-modal]")?.remove(); await ctx.reload(); } catch (err) { if (message) message.textContent = err.message; submit.disabled = false; } return; }
  });
  root.addEventListener("input", (e) => {
    ...
    const pickerAmount = e.target.closest("[data-storage-picker-product-amount]");
    if (pickerAmount) { const { state } = storageProductPickerStateFrom(pickerAmount); if (!state) return; const productId = String(pickerAmount.dataset.storagePickerProductAmount); if (!state.selectedProducts.has(productId)) return; const amount = Number(pickerAmount.value); if (Number.isFinite(amount) && amount > 0) state.selectedProducts.set(productId, amount); return; }
    if (productSearch) { const { content, state } = storageProductPickerStateFrom(productSearch); if (!content || !state) return; state.productQuery = productSearch.value || ""; state.page = 1; renderStoragePickerProductList(content, state); return; }
  });
  root.addEventListener("change", (e) => {
    const selectAll = e.target.closest("[data-storage-picker-select-all]");
    if (selectAll) { const { content, state } = storageProductPickerStateFrom(selectAll); if (!content || !state) return; const { products } = storageProductPickerSelectionState(state); products.forEach((product) => { const id = String(product.id); if (selectAll.checked) { if (!state.selectedProducts.has(id)) state.selectedProducts.set(id, 1); } else state.selectedProducts.delete(id); }); renderStoragePickerProductList(content, state); return; }
    ...
    const id = String(productCheckbox.value); const row = productCheckbox.closest(".warehouse-product-picker-item"); const amountInput = row?.querySelector("[data-storage-picker-product-amount]"); if (productCheckbox.checked) { const amount = Number(amountInput?.value) || 1; state.selectedProducts.set(id, amount); if (amountInput) amountInput.disabled = false; } else { state.selectedProducts.delete(id); if (amountInput) amountInput.disabled = true; }
    syncStoragePickerFooter(content, state);
  });
