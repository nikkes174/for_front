...
export function bindWarehouse(root, ctx) {
  ...
  if (storageId) {
    ...
    try {
      ...
      root.querySelector("[data-storage-products]").textContent = err.message;
    }
    return;
  }
  const del = e.target.closest("[data-storage-delete]"); if (del) { const rows = await api.storages(ctx.org.id); const row = rows.find((r) => Number(r.id) === Number(del.dataset.storageDelete)); if (row && confirm(`Удалить склад «${row.name}»?`)) { try { await api.deleteStorage(row.id); await ctx.reload(); } catch (err) { root.querySelector("[data-message]").textContent = err.message; } } return; } });
  ...
  root.addEventListener("change", (e) => { const subdivisionFilterOption = e.target.closest("[data-storage-subdivision-filter-option]"); if (subdivisionFilterOption) { applyStorageSubdivisionFilter(root); return; } const amount = e.target.closest("[data-storage-product-amount]"); if (amount) selected.set(amount.dataset.storageProductAmount, Number(amount.value)); const check = e.target.closest("[data-storage-product-select]"); if (check && !check.checked) selected.delete(check.dataset.storageProductSelect); });
